...
ssh -T txadmin@${NEW_MAIN_NODE_HOST} ls /tmp > /dev/null
...
if [ $FAILED_NODE_ID -ne $OLD_PRIMARY_NODE_ID ]; then
    ...
    ssh -T txadmin@$OLD_PRIMARY_NODE_HOST "
...
ssh -T  \
    txadmin@${NEW_MAIN_NODE_HOST} ${PGHOME}/bin/pg_ctl -D ${NEW_MAIN_NODE_PGDATA} -w promote
